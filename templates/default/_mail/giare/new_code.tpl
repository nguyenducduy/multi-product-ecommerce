...
<table width="100%" cellspacing="0" cellpadding="0" style="font:normal 15px/25px Arial;">
  <tr>
    <td><span style="font:16px Arial; line-height:25px">Chào <b>{$formData.gender} {$formData.fname}</b>,<br/>
            {$formData.gender|capitalize:true} {$formData.name} vừa nhận thêm {$formData.dem} mã dự thưởng từ <b> chương trình mua sản phẩm giá 1000đ </b> từ <span style="color:#00a1e6">dienmay.com</span> trong ngày {$formData.date}</td>
<tr>
	<td> Mã số nhận thêm hôm nay của {$formData.gender} là:  {$formData.code} </td>
</tr>
  </tr>
  <tr>
