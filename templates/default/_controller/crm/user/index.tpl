			...
			<form class="form-inline" action="" method="post" onsubmit="return confirm('Are You Sure ?');">
				...
				{foreach item=user from=$users}
						...
						<tr>
                            ...
                            <td><a target="_blank" title="Go to this user's home" href="{$conf.rooturl}{$user->screenname}">{$user->screenname}</a></td>
							<td align="center"><span class="label">{$user->groupname($user->groupid)}</span></td>
							<td>{$user->fullname}</td>
							<td>{$user->ipaddress}</td>
							<td>{$user->datecreated|date_format}</td><td><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl_admin}user/edit/id/{$user->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i></a> &nbsp;
