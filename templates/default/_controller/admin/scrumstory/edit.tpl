...
<div class="page-header" rel="menu_scrumstory"><h1>{$lang.controller.head_edit}</h1></div>

<div class="navgoback"><a href="{$smarty.session.backaction}">{$lang.default.formBackLabel}</a></div>

<form action="" method="post" name="myform" class="form-horizontal">
<input type="hidden" name="ftoken" value="{$smarty.session.scrumstoryEditToken}" />
	...
	<div class="control-group" id="divstid">
		<label class="control-label" for="fsiid">{$lang.controller.labelSiid} </label>
		<div class="controls">
            <select name="fsiid" id="fsiid" >
                <option value="0">----</option>        
                <option value="0">Back Log</option>                
                {if $listIteration|@count > 0}
                {foreach from=$listIteration item=iteration}                
                <option value="{$iteration->id}" {if $iteration->id == {$formData.fsiid}}selected="selected"{/if}>{$iteration->name}</option>
                {/foreach}
                {/if}
            </select>
        </div>
	...
	<div class="control-group">
		...
		<div class="controls"><input type="text" name="fasa" id="fasa" value="{$formData.fasa|@htmlspecialchars}" class="input-mini"></div>
	</div>
	...
	<div class="control-group">
		<label class="control-label" for="ftag">{$lang.controller.labelTag}</label>
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <input type="text" class='inputdatepicker' name="fdatecompleted" id="fdatecompleted" value="{$formData.fdatecompleted|date_format:"d/m/yy"}" ></div>
    </div>
	...
	<div class="control-group">
		...
		<div class="controls">
            <select name="fcategoryid" id="fcategoryid" >
                ...
                <option value="{$storycate->id}" {if $storycate->id == {$formData.fcategoryid}}selected{/if}>{$storycate->name}</option>
                {/foreach}
                {/if}
            </select>
        </div>
        ...
        <div class="controls">
            <select name="fsssid" id="fsssid">
                ...
                {foreach $scrumsession as $value}
                    <option value='{$value->id}' {if $value->id == $formData.fsssid} selected = 'selected' {/if}>{$value->name}</option>
                {/foreach}
            </select>
        </div>
    </div>
	...
	<div class="control-group">
		...
		<div class="controls"><select name="fpriority" id="fpriority" >
                <option value="0">----</option> 
            ...
            <select name="flevel" id="flevel" >
                ...
                {if $level|@count > 0}
                    {foreach $level as $k=>$v}
                        <option value="{$k}" {if $k == {$formData.flevel}}selected="selected"{/if}>{$v}</option>
                    {/foreach}
                {/if}
            </select>
        </div>
