	...
	<table class="table table-bordered">
		...
		<tbody>
			...
			<tr>
				...
				<td><input type="checkbox" {foreach item=item key=ncid from=$formData.fnewscategory}{if $newscategory->id == $ncid && $item == 'change'}checked="checked"{/if}{/foreach} value="change" name="fnewscategory[{$newscategory->id}]" id="cg_{$newscategory->id}" onclick="clickfunc('cg_{$newscategory->id}' , 'cv_{$newscategory->id}')"/></td>
			</tr>
			<tr>
			</tr>
			{/foreach}
