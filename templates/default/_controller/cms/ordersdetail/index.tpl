	...
	<div class="tab-content">
				...
				{if $ordersdetails|@count > 0}
					<thead>
						<tr>
						   <th width="40"><input class="check-all" type="checkbox" /></th>
							...
							<th>{$lang.controller.labelOptions}</th>
							<th width="140"></th>
						</tr>
					</thead>
					...
					{foreach item=ordersdetail from=$ordersdetails}
						...
						<tr>
							...
							<td>{$ordersdetail->pid}</td>
							<td>{$ordersdetail->pricesell}</td>
							<td>{$ordersdetail->pricediscount}</td>
							<td>{$ordersdetail->pricefinal}</td>
							<td>{$ordersdetail->quantity}</td>
							<td>{$ordersdetail->options}</td>
							
							<td><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl}{$controllerGroup}/{$controller}/edit/id/{$ordersdetail->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i> {$lang.default.formEditLabel}</a> &nbsp;
								<a title="{$lang.default.formActionDeleteTooltip}" href="javascript:delm('{$conf.rooturl}{$controllerGroup}/{$controller}/delete/id/{$ordersdetail->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i> {$lang.default.formDeleteLabel}</a>
							</td>
						</tr>
		...
		<div class="tab-pane" id="tab2">
			<form class="form-inline" action="" method="post" style="padding:0px;margin:0px;" onsubmit="return false;">
				...
				{$lang.controller.labelPricesell}: <input type="text" name="fpricesell" id="fpricesell" value="{$formData.fpricesell|@htmlspecialchars}" class="input-mini" /> - 

				{$lang.controller.labelPricefinal}: <input type="text" name="fpricefinal" id="fpricefinal" value="{$formData.fpricefinal|@htmlspecialchars}" class="input-mini" /> - 
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl + controllerGroup + "/ordersdetail/index";
		...
		var pricesell = $('#fpricesell').val();
		if(parseInt(pricesell) > 0)
		{
			path += '/pricesell/' + pricesell;
		}

		var pricefinal = $('#fpricefinal').val();
		if(parseInt(pricefinal) > 0)
		{
			path += '/pricefinal/' + pricefinal;
		}
