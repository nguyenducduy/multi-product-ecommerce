...
<div class="event-header" rel="menu_event"><h1>{$lang.controller.head_list}</h1></div>  
...
<div class="tabbable">
	<ul class="nav nav-tabs">
		...
		<li class="pull-right"><a class="pull-right btn" href="{$conf.rooturl}{$controllerGroup}/eventtheme">Theme Manager</a></li>
	...
	<div class="tab-content">
		<div class="tab-pane active" id="tab1">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				<input type="hidden" name="ftoken" value="{$smarty.session.eventBulkToken}" />
				...
				{if $pages|@count > 0}
					...
					{foreach item=page from=$pages}
						<tr>
							...
							<td>{if $page->iscounter == 1}<span class="label label-success">Yes</span>{else}<span class="label">No</span>{/if}</td>
							<td>{$page->starttime|date_format:"%d/%m/%Y"}</td>
							<td>{$page->endtime|date_format:"%d/%m/%Y"}</td>
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl + controllerGroup + "/event/index";
