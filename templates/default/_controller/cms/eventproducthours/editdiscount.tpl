<ul class="breadcrumb">
    ...
    <li class="active">{$lang.controller.head_edit_promotion}</li>
</ul>

<div class="page-header" rel="menu_eventproducthours"><h1>{$lang.controller.head_edit_promotion}</h1></div>
...
<form action="" method="post" name="myform" class="form-horizontal" enctype="multipart/form-data">
    <input type="hidden" name="ftoken" value="{$smarty.session.editDiscountToken}" />
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <select name="fstatus">
                <option value="1" {if $formData.fstatus == 1}selected='selected'{/if}}>Enable</option>
                <option value="0" {if $formData.fstatus == 0}selected='selected'{/if}}>Disable</option>              
            </select>
        </div>
    </div>
