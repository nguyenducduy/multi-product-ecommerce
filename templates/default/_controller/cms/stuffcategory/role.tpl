...
<div class="page-header" rel="menu_stuffcategory"><h1>{$lang.controller.head_list_role}</h1></div>
	...
	<div class="tab-content">
					...
					<thead>
						<tr>
						   <!-- <th width="40"><input class="check-all" type="checkbox" /></th> -->
							...
							<th><a href="{$filterUrl}sortby/objectid/sorttype/{if $formData.sortby eq 'objectid'}{if $formData.sorttype|upper neq 'DESC'}DESC{else}ASC{/if}{/if}">{$lang.controller.labelHaveStuffcategory}</a></th>
					...
					<tbody>
						{foreach item=groups from=$groupList}
							...
							<tr>
								...
								<td>{if $roleuser->stuffcategory->parent|@count > 0}
									{foreach item=parent from=$roleuser->stuffcategory->parent}
									<b>{$parent.nc_name}</b> &raquo;
									{/foreach}
									{/if}
									<span class="label label-info">{$roleuser->stuffcategory->name}</span>
								</td>
								...
								{/if}</td>
								<td><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl}{$controllerGroup}/{$controller}/roleedit/uid/{$roleuser->uid}/scid/{$roleuser->objectid}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i> {$lang.default.formEditLabel}</a> &nbsp;
								<a title="{$lang.default.formActionDeleteTooltip}" href="javascript:delm('{$conf.rooturl}{$controllerGroup}/{$controller}/roledelete/uid/{$roleuser->uid}/scid/{$roleuser->objectid}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i> {$lang.default.formDeleteLabel}</a></td>
							</tr>
							{else}
							<tr>
		...
		<div class="tab-pane" id="tab2">
			<form class="form-inline" action="" method="post" style="padding:0px;margin:0px;" onsubmit="return false;">
				...
				{foreach from=$stuffcategoryList item=category}
