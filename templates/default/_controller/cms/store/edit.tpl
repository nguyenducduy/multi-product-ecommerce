...
<div class="page-header" rel="menu_store"><h1>{$lang.controller.head_edit}</h1></div>

<div class="navgoback"><a href="{$redirectUrl}">{$lang.default.formBackLabel}</a></div>

<form action="" method="post" name="myform" class="form-horizontal" enctype="multipart/form-data">
<input type="hidden" name="ftoken" value="{$smarty.session.storeEditToken}" />
    ...
    <div class="control-group">
        <label class="control-label" for="fimage">{if !empty($formData.fimage)}<img src="{$conf.rooturl}uploads/store/{$formData.fimage}" />{/if}</label>
