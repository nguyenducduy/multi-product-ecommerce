...
<div class="page-header" rel="menu_job"><h1>{$lang.controller.head_edit}</h1></div>

<div class="navgoback"><a href="{$redirectUrl}">{$lang.default.formBackLabel}</a></div>

<form action="" method="post" name="myform" class="form-horizontal" enctype="multipart/form-data">
<input type="hidden" name="ftoken" value="{$smarty.session.jobEditToken}" />
	...
	<div class="control-group">
		<label class="control-label" for="fimage">{$lang.controller.labelImage}<img src="{$conf.rooturl}uploads/job/{$formData.fimage}"/></label>
		<div class="controls"><input type="file" name="fimage" id="fimage" value="{$formData.fimage|@htmlspecialchars}" class="input-xlarge"></div>
	</div>
