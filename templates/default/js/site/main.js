...
function SBclose() {
  ...
  if (
    $('#sb-title-inner').text() == 'Sửa hình 360' ||
...
*/
  ...
  $('#fsitebooksearchtext')
    .autocomplete(rooturl + 'search/suggest', {
      width: 462,
      scroll: true,
      scrollHeight: 500,
      highlight: false,
  ...
  $('#fsitecompare')
    .autocomplete(rooturl + 'productcompare/suggest', {
      width: 265,
      scroll: true,
      scrollHeight: 300,
      highlight: false,
  ...
  if ($('#fcurrentregion').length > 0) {
    ...
    $('#finstallmentmonth').change(function () {
      ...
      if (m != '') {
        $.post(
          rooturl + 'product/prepaid',
          {
            m: m,
            r: $('#fcurrentregion').val(),
            o: $('.installrightsp .productname').attr('id'),
            c: $('#fsegmentpercent').val(),
          },
          function (data) {
            if (data) {
              if (data.success == 1) {
  ...
  $('.inputdatepicker, .inputdatepickersmall')
  ...
  $('.video-small .popupvideo img')
    ...
    .click(function () {
      ...
      if (url) {
        ...
        $('#runyoutubeurl').html(
          '<iframe width="420" height="315" src="' + url + '?autoplay=1" frameborder="0" allowfullscreen></iframe>'
        )
  ...
  function checkaccessory() {
    if ($('.chkaccessory').length > 0) {
      ...
      if ($('#accessorybox').attr('rel') != '') {
        ...
        $.post(
          rooturl + 'product/updateaccessory',
          { pid: $('#accessorybox').attr('rel'), acc: checkedid, promo: promo },
          function (success) {
            if (success && success.mainprice) {
              ...
              if (success.saving && success.saving != '') {
                if (success.numproduct > 0)
                  htmlassign +=
                    '<div class="accessrow" id="selectnumber" style="display: none;">Bạn vừa chọn: <strong>' +
                    success.numproduct +
                    '</strong> phụ kiện</div>'
                htmlassign +=
                  '<div class="accessrow" id="priceassessory">Giá mua lẻ: <strong>' +
                  success.promotionprices +
                  'đ</strong></div>'
                htmlassign +=
                  '<div class="accessrow" id="priceretail">Giá mua kèm: <span>' + success.mainprice + 'đ</span></div>'
                htmlassign +=
                  '<div class="accessrow" id="pricesaving">Tiết kiệm: <span>' + success.saving + 'đ</span></div>'
                if (success.numproduct > 0)
                  htmlassign +=
                    '<div class="accessrow accbtn"><a href="javascript:void(0)" id="addaccessoryproduct" onclick="addaccessorytocart()">Mua kèm ' +
                    success.numproduct +
                    ' sản phẩm đã chọn</a></div>'
                else
                  htmlassign +=
                    '<div class="accessrow accbtn"><a href="javascript:void(0)" id="addaccessoryproduct" onclick="addaccessorytocart()">Mua sản phẩm đã chọn</a></div>'
              } else {
                if (success.numproduct > 0)
                  htmlassign +=
                    '<div class="accessrow" id="selectnumber" style="display: none;">Bạn vừa chọn: <strong>' +
  ...
  //Slider page product /////////////////////////////////////////
  if (
    typeof ispriceajax !== 'undefined' &&
    ispriceajax == 1 &&
    $('.loadprice').length > 0 &&
    (getParameterByName('color') <= 0 || getParameterByName('color') == '')
  ) {
    var str20first = ''
    var str30next = ''
    var str50last = ''
          ...
          if (datajs && datajs.data) {
            $.each(datajs.data, function (i) {
              ...
              if (objassign.find('.priceold').length > 0) {
                ...
                if (
                  objassign.find('div.pricedienmay').length > 0 &&
                  objassign.find('div.dienmay').length > 0 &&
                  objassign.find('div.genuine').length == 0
                ) {
                  objassign
                    .find('div.dienmay')
                    .before(
                      '<div class="genuine"><div class="textgenuine">Giá chính hãng:</div><div class="pricegenuine"><span>' +
                        datajs.data[i].sell +
                        ' đ</span></div></div>'
                    )
              ...
              if (
                ...
                objassign.find('.priceold').length == 0 &&
                objassign.find('div.pricenew').length > 0
              ) {
                  ...
                  if (datajs && datajs.data) {
                    $.each(datajs.data, function (i) {
                      ...
                      if (parseInt(datajs.data[i].isdiscount) == 1 && objassign.find('.salepercent').length > 0) {
                        ...
                        parseInt(datajs.data[i].isdiscount) != 1 &&
                        objassign.find('.salepercent').length > 0
                      ) {
                        objassign.find('.salepercent').remove()
                      }
                      if (
                        ...
                        objassign
                          .find('div.pricenew')
                          .after('<div class="priceold">' + datajs.data[i].sell + ' đ</div>')
                      }
                    })
                    if (str50last != '') {
                      $.ajax({
                          ...
                          if (datajs && datajs.data) {
                            $.each(datajs.data, function (i) {
                              ...
                              if (objassign.find('.priceold').length > 0) {
                                ...
                                objassign
                                  .find('div.pricenew')
                                  .after('<div class="priceold">' + datajs.data[i].sell + '</div>')
                              }
                              if (
                                parseInt(datajs.data[i].isdiscount) == 1 &&
                                objassign.find('.salepercent').length > 0
  ...
  $('.userpromo0209').live('click', function () {
    if ($(this).attr('rel') != '')
      Shadowbox.open({
        ...
        player: 'iframe',
        options: { modal: true },
        title: 'Danh sách khách hàng mua sản phẩm',
        height: 430,
        width: 750,
      })
  })
...
function loadinstock(idcolor) {
  $.post(
    rooturl + 'product/checkstorestock',
    { fpid: idcolor },
    function (data) {
      //
      if (data) {
...
function doSitebooksearchtextpress(e) {
  if (e.keyCode == 13) {
    ...
    if (autocompleteUrl.length == 0) $('#fsitebooksearchsubmit').trigger('click')
    else location.href = autocompleteUrl
  }
}
...
function fsitebooksearchtextAutoCompleteFormatItem(row) {
  ...
  if (resulttype == 'product' || resulttype == 'news' || resulttype == 'stuff' || resulttype == '') {
    ...
    output +=
      '<div class="sitesearch-autocomplete-seperator"><div class="sitesearch-autocomplete-textsep">' +
      value +
      '</div></div>'
  }
            ...
            if (result.enablevalue && result.disablevalue) {
              ...
              if (result.convertenablevalue && result.convertdisablevalue) {
                ...
                newtext = newtext.replace(
                  'rel="' + result.relconvertdisablevalue + '"',
                  'rel="' + result.relconvertenablevalue + '"'
                )
              }
            } else {
...
function sendReview(id, parentid, check, reply) {
  ...
  if (check == 1 || check == 0) {
    ...
    if (content == '') {
      if (check == 1) nalert('Vui lòng nhập nội dung bình luận', 'Lỗi')
      else {
        ...
        success: function (html) {
          ...
          if (html == '6') {
            ...
            if (reply == 1 || reply == -1) {
              ...
              $('.notifireview').html(
                '<div class="notify-bar-icon"><img src="' +
                  rooturl +
                  'templates/default/images/notify/success.png" alt="success"></div><div class="notify-bar-button"><a href="javascript:void(0);" onclick="javascript:$(this).parent().parent().fadeOut();" title="close"><img src="' +
                  rooturl +
                  'templates/default/images/notify/close-btn.png" border="0" alt="close"></a></div><div class="notify-bar-text"><p>Ý kiến của bạn đã được đăng. Cám ơn bạn đã đóng góp cho dienmay.com</p></div>'
              )
        ...
        success: function (html) {
          ...
          if (data.comment == '6') {
            ...
            if (reply == 1 || reply == -1) {
              ...
              $('.review-product').prepend(html)
              $('.combtn').html(
                '<div class="combtn"><a href="javascript:void(0)" class="btn-blues" onclick="sendReview(' +
                  pid +
                  ' , 0 , 1 , -1)">Gửi bình luận</a></div>'
              )
...
function loadReview(id, order) {
  $.ajax({
    ...
    success: function (html) {
      $('#comment').html(html)
      var countcomment =
        parseInt($('.commentbar h2 span').html()) > 0 ? '(' + $('.commentbar h2 span').html() + ')' : ''
    ...
    $.ajax({
      ...
      success: function (html) {
        if (html == 'done') {
          ...
          currentthumbup = currentthumbup + 1
          //$('#like'+reviewid).html(currentthumbup);
          $('#likecom' + reviewid).html(
            '<span>Thích (<span id="like' + reviewid + '">' + currentthumbup + '</span>)</span>'
          )
        }
      },
    })
...
if (typeof hideMenu != 'undefined' && hideMenu == '1') {
  ...
  $('.dropdown-menu-aim').removeClass('showhide')
}
...
function addaccessorytocart() {
  if ($('.chkaccessory:checked').length > 0) {
    $.post(
      rooturl + 'cart/addcartajax',
      { id: $('#accessorybox').attr('rel') },
      function (data) {
        if (data && data.success) {
          if (data.success == 1) {
            $('#accessoryparentbox').css('position', 'relative')
            $('#accessoryparentbox').append(
              '<p id="accesoryalert" style="background: #00A1E6;color: #FFFF00;display: block;padding: 10px 5px;position: absolute;right: 2px;text-transform: uppercase;top: 50%;width: 350px;z-index: 999;">Sản phẩm của bạn đã được thêm vào giỏ hàng</p>'
            )
            ...
            $('#accessoryparentbox').css('position', 'relative')
            $('#accessoryparentbox').append(
              '<p id="accesoryalert" style="background: #00A1E6;color: #FFFF00;display: block;padding: 10px 5px;position: absolute;right: 2px;text-transform: uppercase;top: 50%;width: 350px;z-index: 999;">Không thể thêm sản phẩm vào giỏ hàng, vui lòng chờ 5 giây</p>'
            )
            $('#accesoryalert').hide(3000)
          }
        }
...
function initinternalbar() {
  ...
  if (reporttype.length > 0 || parseInt(objectid) > 0) initinternalbar_summary(reporttype, objectid, reporturl)
...
\
    ...
    'top_notification.png" /><span class="badge hide">0</span></a>\
		          <div class="topitempanel hide" style="height: auto;">\
		              ...
		              <div class="viewall"><a href="' +
    rooturl +
    'profile/notification" title="Xem tất cả các thông báo">Xem tất cả</a></div>\
		          </div><!-- .topitemdata -->\
		      </div><!-- end #topnotification -->\
		  </div><!-- end #topnotify -->\
		'
  if (objectid && parseInt(objectid) > 0 && reporturl != '')
    tophtml +=
      '<a class="popupbread" href="javascript:void(0)" title="View Report" onclick="stat_open(\'' +
      reporturl +
      '\')">ID: ' +
      objectid +
      '</a> '
  else if (objectid && parseInt(objectid) > 0)
    tophtml += '<a class="popupbread" href="javascript:void(0)" title="View Report">ID: ' + objectid + '</a> '
  else if (objectid && parseInt(objectid) == -1 && reporturl != '')
    tophtml +=
      '<a class="popupbread" href="javascript:void(0)" title="View Report" onclick="stat_open(\'' +
      reporturl +
      '\')">Root Report</a> '
  ...
  tophtml +=
    '<a href="javascript:void(0)" onclick="$(\'#popupeditinline\').hide()" id="hidepopup" title="Close this bar"><img src="' +
    imageDir +
    'stat/close.png" alt="Close" /></a> '

  if (objectid && parseInt(objectid) > 0)
    tophtml +=
      '<a href="' +
      editurl +
      '" id="editinlineedit" title="Go to edit page"><img src="' +
      imageDir +
      'stat/edit.png" alt="Edit" /></a>\
		<a href="' +
      refreshurl +
      '" id="editinlinerefresh" title="View of Customer"><img src="' +
      imageDir +
      'stat/customerview.png" alt="Customer View" /></a>\
		</div>'
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      if (gallerycolor.length != 0) {
        ...
        html +=
          '<img data-zoom-image="' +
          gallerycolor[0].imagelarge +
          '" src="' +
          gallerycolor[0].imagemedium +
          '" id="zoom">'
        html += '</div>'
        ...
        $.each(gallerycolor, function (i) {
          if (i < numberimg) {
            html +=
              '<a data-zoom-image="' +
              gallerycolor[i].imagelarge +
              '" data-image="' +
              gallerycolor[i].imagemedium +
        ...
        if ($('#loadpromotionlist').length > 0 && productid) {
          ...
          if (firstpromoobj && firstpromoobj.val() != '' && typeof firstpromoobj.val() != 'undefined') {
            ...
            if (parseInt(sell.replace(/,/g, '')) > 0 && parseInt(discount.replace(/,/g, '')) > 0) {
              if (
                parseInt(discount.replace(/,/g, '')) > 0 &&
                isdiscount == 1 &&
                parseInt(sell.replace(/,/g, '')) > parseInt(discount.replace(/,/g, ''))
              ) {
...
function ischange() {
  ...
  if (ischange == 'false') {
    ...
    html +=
      '<a href="javascript:void()" class="ismember" rel="true" title="" onclick="ischange()">Bình luận như là khách ?</a>'
    ...
    html +=
      '<a style="float:right" href="' + rooturl + 'thanh-vien/quen-mat-khau" title="" target="_blank">Quên mật khẩu</a>'
    html +=
      '<a href="javascript:void(0)" class="btn-blues" onclick="sendReview(' +
      reviewid +
      ' , 0 , 2 , -1)">Gửi bình luận và đăng nhập</a>'
  } else {
    var html = ''
    html += '<div class="notifireviewthumb"></div>'
    html +=
      '<a href="javascript:void()" class="ismember" rel="false" title="" onclick="ischange()">Đã là thành viên điện máy? Đăng nhập &#187;</a>'
