...
<input type="hidden" name="ftoken" value="{$smarty.session.homepageAddToken}" />
	...
	<div id="homepageproduct" style="display:none"><h1>Sản phẩm khuyến mãi được chọn</h1><table class="table" id="choosehomepage"><thead><tr><th></th><th>Danh mục</th><th>Tên sản phẩm</th><th>Giá</th><th>Số lượng</th><th></th></tr></thead><tbody></tbody></table></div>
	...
	if($('#barcode').val() != '' || $('#pname').val() != '')
		{
			...
			$.ajax({
				type : "post",
				dataType : "html",
				url : "/cms/homepage/loadpromotionajax",
				data : dataString,
