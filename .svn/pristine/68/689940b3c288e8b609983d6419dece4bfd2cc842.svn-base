		...
		<table class="table table-bordered tablesorter" id="data">
			...
			<tbody>
				...
				<tr>
					...
					<td><a rel="{$conf.rooturl}stat/report/productdetail/forecastproductajax?id={$rowdata.id}" href="javascript:void(0)" class="viewpopup"><span id="Q{$smarty.foreach.foo1.index+2}" class="label label-info">{$rowdata.tongsoluongbansieuthi}</span></a></td>

					{if $isedit == 0}<td><span id="R{$smarty.foreach.foo1.index+2}">{$rowdata.soluongbandk.value}</span></td>{else}<td dataid="SS{$smarty.foreach.foo1.index+2}" id="{$rowdata.soluongbandk.name}" class="editable" rel="R{$smarty.foreach.foo1.index+2}">{$rowdata.soluongbandk.value}</td>{/if}

					{if $isedit == 0}<td><span id="S{$smarty.foreach.foo1.index+2}">{$rowdata.soluongnhapdk.value}</span></td>{else}<td id="{$rowdata.soluongnhapdk.name}" data="notchange" number="X{$smarty.foreach.foo1.index+2},Y{$smarty.foreach.foo1.index+2}" class="editable" rel="S{$smarty.foreach.foo1.index+2}">{$rowdata.soluongnhapdk.value}</td><span style="display:none;" rel="S{$smarty.foreach.foo1.index+2}" class="datanew" id="SS{$smarty.foreach.foo1.index+2}" data-formula="IF($O{$smarty.foreach.foo1.index+2}-$R{$smarty.foreach.foo1.index+2} >= $X{$smarty.foreach.foo1.index+2} , 0 , $R{$smarty.foreach.foo1.index+2}-$O{$smarty.foreach.foo1.index+2}+$X{$smarty.foreach.foo1.index+2})"></span>{/if}

					<td><span  id="T{$smarty.foreach.foo1.index+2}" data-formula="$O{$smarty.foreach.foo1.index+2}+$S{$smarty.foreach.foo1.index+2}-$R{$smarty.foreach.foo1.index+2}"></span></td>

					<td><span id="U{$smarty.foreach.foo1.index+2}" data-format="0,0[.]00" data-formula="{if $rowdata.finalprice > 0}{$rowdata.finalprice}*$R{$smarty.foreach.foo1.index+2}{else}{$rowdata.sellprice}*$R{$smarty.foreach.foo1.index+2}{/if}"></span></td>

					<td style="display:none;"><span id="V{$smarty.foreach.foo1.index+2}" data-format="0,0[.]00" data-formula="$U{$smarty.foreach.foo1.index+2} - ($R{$smarty.foreach.foo1.index+2} * {$rowdata.inputpricereal} )"></span></td>
					
					<td><span id="W{$smarty.foreach.foo1.index+2}" data-format="0,0[.]00" data-formula="$U{$smarty.foreach.foo1.index+2} - ($R{$smarty.foreach.foo1.index+2} * $I{$smarty.foreach.foo1.index +2} )"></span></td>

					<span style="display:none;" id="X{$smarty.foreach.foo1.index+2}" data-formula="IF($D{$smarty.foreach.foo1.index+2} > ($F{$smarty.foreach.foo1.index+2} * $P{$smarty.foreach.foo1.index+2}) , $D{$smarty.foreach.foo1.index+2} , ($F{$smarty.foreach.foo1.index+2} * $P{$smarty.foreach.foo1.index+2}) )"></span>

					<span style="display:none;" id="Y{$smarty.foreach.foo1.index+2}" data-formula="$G{$smarty.foreach.foo1.index+2} * $P{$smarty.foreach.foo1.index+2}"></span>
				</tr>
				{/foreach}
				<tr>
					<td>{$datalist|@count + 2}</td>
					<td colspan="7"><b style="text-align: center;">Total</b></td>										
					...
					<td><span data-format="0,0[.]00" class="total" id="U{$datalist|@count + 2}" rel="SUM($U2,$U{$datalist|@count+1})"></span></td>
					<td style="display:none;"><span data-format="0,0[.]00" class="total" id="V{$datalist|@count + 2}" rel="SUM($V2,$V{$datalist|@count+1})"></span></td>
