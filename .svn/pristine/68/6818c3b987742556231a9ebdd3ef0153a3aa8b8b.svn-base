<div id="bookcomment">
			...
			<div class="head">
				<h2 class="left">{$lang.controller.editreviewToBook} "<em>{$myBook->title}</em>"</h2>
			...
			<div id="commentform" class="myform myformwide stylizedform stylizedform_sub">
				...
				<form id="formreview" name="form1" method="get" action="">
					...
					<div class="mentionableinputwrapper">
				 		<textarea class="mentionable" title="{$lang.controller.typeometerTitle}" name="fmessage" id="fmessage" cols="30" rows="8">{$myReview->text}</textarea>
					</div>
					...
					<input type="button" class="submit" id="fsubmit" value="{$lang.controller.editSubmit}" onclick="bookreviewEdit({$myReview->id})" />
		...
		$(document).ready(function()
		{
			...
			$('#fmessage').keyup(function(){
				...
				$('#fmessage-typeometer .typeometer-right').html(cchar + ' k&#237 t&#7921');
