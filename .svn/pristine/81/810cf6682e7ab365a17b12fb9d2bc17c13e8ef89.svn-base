...
<div class="page-header" rel="menu_giftorder"><h1>{$lang.controller.head_edit}</h1></div>

<div class="navgoback"><a href="{$redirectUrl}">{$lang.default.formBackLabel}</a></div>

<form action="" method="post" name="myform" class="form-horizontal">
<input type="hidden" name="ftoken" value="{$smarty.session.giftorderEditToken}" />
	...
	{if $formData.fproductid|count >0}
		{foreach from=$formData.fproductid key=key item=id}
				...
				<input type="hidden" value="{$formData.forderid.$key}" name='forderid[]' />
				<span style="margin-left:30px; color:#3498db">Sản phẩm {$key+1}</span>
				<div class="control-product">
					...
					<div class="control-group">
						<label class="control-label" for="finstock">Số lượng còn lại <span class="star_require">*</span></label>
						<div class="controls"><input type="text" name="finstock[]" id="finstock" value="{$formData.finstock.$key}" class="input-large"></div>
					</div>
					<div class="control-group">
	...
	{else}
		<div class="control-product-group">
			...
			<div class="control-product">
				<div class="control-group">
					<label class="control-label" for="fproductid">Mã sản phẩm <span class="star_require">*</span></label>
					<div class="controls"><input type="text" name="fproductid[]" id="fproductid" value="{$formData.fproductid|@htmlspecialchars}" class="input-small"></div>
				</div>
...
{literal}
  <script type="text/javascript">
        ...
        var i = {/literal}{$numberproduct}{literal};
