...
<input type="hidden" name="ftoken" value="{$smarty.session.homepageEditToken}" />
	...
	<div id="homepageproduct">
		<h1>Sản phẩm khuyến mãi được chọn</h1>
		<table class="table" id="choosehomepage">
			<thead>
				<tr><th></th><th>Danh mục</th><th>Tên sản phẩm</th><th>Giá</th><th>Số lượng</th><th></th></tr>
			</thead>
			<tbody>
...
function chooseFunction(id)
{
	if(id > 0)
	{
		...
		{
			bootbox.alert('{/literal}{$lang.controller.errPromotionChoose}{literal}');
		}
	}
}
