...
class SendMail
{
	...
	public function Send()
	{
		...
		{
			$sesConf = array('accessKey' => $this->registry->setting['amazon']['publickey'], 'privateKey' => $this->registry->setting['amazon']['privatekey']);
			...
			try 
			{
				...
				/*
				$toList = array('to' => array('tuanmaster2002@yahoo.com' => 'Vo Duy Tuan B'),
								'bcc' => array('webmaster@vinabranding.com' => 'Vinabranding', 'admin@reader.vn' => 'Reader')
				);
				*/
				
				if(count($toList) > 0)
				{
					$result = $this->mailer->sendEmail($toList, array('source' => array($this->fromEmail => $this->fromName)),$this->mailSubject, array('html' => $this->mailContents));
				}
				else
				{
					$result = false;
