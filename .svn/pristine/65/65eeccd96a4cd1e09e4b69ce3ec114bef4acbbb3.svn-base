...
<html lang="en">
	...
	<body>
		...
		<div id="apidocbaseurl">BASE_URL: <span>http://example.com</span></div>
		...
		{if $groupList|@count > 0}
			{foreach item=group from=$groupList}
				<div class="apigroup" id="apigroup-{$group->id}">
					<div class="apighead">
						<h2><a href="javascript:void(0)" onclick="$('#apigroup-{$group->id} .apigbody').slideToggle();" title="Show/Hide This Group">{$group->name}</a></h2>
					...
					<div class="apigbody">
						{foreach item=r from=$group->requestList}
							<div class="apirequest apirequest_lite apirequest_{$r->getMethodName()}" id="apirequest-{$r->id}">
								<div class="apirhead">
									<div class="apirmethod" onclick="$('#apirequest-{$r->id}').toggleClass('apirequest_lite')">{$r->getMethodName()}</div>
									<div class="apirname"><a href="javascript:void(0)" onclick="$('#apirequest-{$r->id}').toggleClass('apirequest_lite')">{$r->name}</a></div>
											...
											<table>
												...
												<tbody>
													{foreach item=param from=$r->paramList}
														<tr class="{if $param->isrequired == 1}paramrequired{/if}">
															<td>{$param->name}</td>
															<td>{$param->getDatatypeName()}</td>
															<td><input type="text" class="param" id="param-{$param->name}" value="" {if $param->isrequired == 1}placeholder="(Required)"{/if} /></td>
															<td>{$param->summary}</td>
														</tr>
													{/foreach}
												</tbody>
