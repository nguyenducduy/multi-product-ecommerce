...
Class Core_Backend_Notification_FeedCommentLikeFollow extends Core_Backend_Notification
{
	public $feedpath = '';
	public $summary = '';
	...
	{
		parent::__construct(); 
		$this->type = parent::TYPE_FEED_COMMENT_LIKE_FOLLOW;               
	}
	...
	*/
	...
	public function showDetail($display = false)
	{
		...
		if($this->summary != '')
		{
			$this->summary = $registry->lang['controller']['notificationFeedCommentLikeFollowLinkTextSummary'] . ' <em>' . $this->summary . '</em>' . $registry->lang['controller']['notificationFeedCommentLikeFollowLinkTextSummarySuffix'];
		}
		else
		{
		...
		$out = '<a class="notifyitem" href="'.$this->feedpath.'" title="'.$registry->lang['controller']['notificationFeedDetailTitle'].'">'.$actorInfo .$registry->lang['controller']['notificationFeedCommentLikeFollowText'].' '.$this->summary.'</a>';
