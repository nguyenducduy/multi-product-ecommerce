...
Class Core_Message extends Core_Object
{
	const TYPE_MAIL = 1;
    ...
    */
    ...
    public function delete($deleteuserid)
    {
		...
		elseif($this->uidfirstdelete != $deleteuserid)
		{
	        ...
	        {
	        	$myMessageText = new Core_MessageText();
	...
	public static function getMessages($formData, $sortby, $sorttype, $limit = '', $countOnly = false)
	{
		...
		{
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'm.mt_id NOT IN (SELECT mts.mt_id FROM ' . TABLE_PREFIX . 'message_text_status mts WHERE mts.u_id = '.$registry->me->id.' AND mts.mts_status > '.Core_MessageTextStatus::STATUS_INBOX .')';
		}

		if(isset($formData['fisintrash']))
		{
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'm.mt_id IN (SELECT mts.mt_id FROM ' . TABLE_PREFIX . 'message_text_status mts WHERE mts.u_id = '.$registry->me->id.' AND mts.mts_status = '.Core_MessageTextStatus::STATUS_INTRASH .')';
		}

		if(isset($formData['fisinfolder']))
		{
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'm.mt_id IN (SELECT mts.mt_id FROM ' . TABLE_PREFIX . 'message_text_status mts WHERE mts.u_id = '.$registry->me->id.' AND mts.mts_status = '.Core_MessageTextStatus::STATUS_INFOLDER .' AND mts.mts_folderid = '.(int)$formData['fisinfolder'].')';
		}

		if(isset($formData['fisstarred']))
		{
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'm.mt_id IN (SELECT mts.mt_id FROM ' . TABLE_PREFIX . 'message_text_status mts WHERE mts.u_id = '.$registry->me->id.' AND mts.mts_isstarred = 1 AND mts.mts_status < '.Core_MessageTextStatus::STATUS_INTRASH.')';
		}
   ...
   */
   	public static function countMyMessage($userid, $start, $end)
   	{
   		...
   		return $db->query($sql, array((int)$userid, (int)$start, (int)$end))->fetchColumn(0);
	}
	...
	public function getFullRecipients()
	{
		...
		$sql = 'SELECT GROUP_CONCAT(u_id_to SEPARATOR ",") AS recipients
				...
				WHERE mt_id = ?
				GROUP BY mt_id';
		$recipientstring = $this->db->query($sql, array($this->mtid))->fetchColumn(0);
