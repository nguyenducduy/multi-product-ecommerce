...
Class Core_Backend_Feed_GroupJoin extends Core_Backend_Feed
{
	...
	{
		parent::__construct(); 
		$this->type = parent::TYPE_GROUP_JOIN;               
	}
	...
	public function showDetail($display = false)
	{
		...
		if(!empty($this->receivermore))
		{
			...
			$out = '<div class="text"><a class="username tipsy-hovercard-trigger" data-url="'.$this->actor->getHovercardPath().'" href="'. $this->actor->getUserPath().'" title="">'.$this->actor->fullname.$this->actor->getNameIcon().'</a> tham gia <a class="tipsy-hovercard-trigger" data-url="'.$this->receiver->getHovercardPath().'" href="'. $this->receiver->getUserPath().'" title="">'.$this->receiver->fullname.$this->receiver->getNameIcon().'</a>'.$morefullname.'.</div>
		...
		elseif(!empty($this->actormore))
		{
			...
			$out = '<div class="text"><a class="username tipsy-hovercard-trigger" data-url="'.$this->actor->getHovercardPath().'" href="'. $this->actor->getUserPath().'" title="">'.$this->actor->fullname.$this->actor->getNameIcon().'</a>'.$morefullname.' &#273;&#7873;u tham gia <a class="tipsy-hovercard-trigger" data-url="'.$this->receiver->getHovercardPath().'" href="'. $this->receiver->getUserPath().'" title="">'.$this->receiver->fullname.$this->receiver->getNameIcon().'</a>.</div>
		...
		{
			$out = '<div class="text"><a class="username tipsy-hovercard-trigger" data-url="'.$this->actor->getHovercardPath().'" href="'. $this->actor->getUserPath().'" title="">'.$this->actor->fullname.$this->actor->getNameIcon().'</a> tham gia <a class="tipsy-hovercard-trigger" data-url="'.$this->receiver->getHovercardPath().'" href="'. $this->receiver->getUserPath().'" title=""><img src="'.$this->receiver->getSmallImage().'" alt="" style="width:16px;" /> '.$this->receiver->fullname.$this->receiver->getNameIcon().'</a></div>
