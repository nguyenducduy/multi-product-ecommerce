...
<div class="page-header" rel="menu_productcolor"><h1>{$lang.controller.head_add}</h1></div>

<div class="navgoback"><a href="{$conf.rooturl}{$controllerGroup}/{$controller}">{$lang.default.formBackLabel}</a></div>

<form action="" method="post" name="myform" class="form-horizontal">
<input type="hidden" name="ftoken" value="{$smarty.session.productcolorAddToken}" />
	...
	<div class="control-group">
		...
		<div class="controls">
			<input type="text" name="fcode" value="{$formData.fcode|@htmlspecialchars}" class="input-large" />
		</div>
	</div>
