...
Class Core_Notification extends Core_Object
{
	...
	const TYPE_BLOG_COMMENT = 30;
	const TYPE_BLOG_COMMENT_FOLLOW = 31;
	...
	*/
	...
	public function updateData()
	{
		...
		$sql = 'UPDATE ' . TABLE_PREFIX . 'notification
				SET n_dateupdated = ?
        		WHERE n_id =  ? ';
		$rowCount = $this->db->query($sql, array( $this->dateupdated,$this->id))->rowCount();
	...
	*/
	public static function updateReadStatus($receiveid)
	{
		...
		$sql = 'UPDATE ' . TABLE_PREFIX . 'notification
        		...
        		WHERE u_id_receive =  ?
					AND n_dateupdated = 0';
		$rowCount = $db->query($sql, array( time(), (int)$receiveid))->rowCount();
	}
	...
	private function getData($id)
	{
		$sql = 'SELECT *
				...
				INNER JOIN ' . TABLE_PREFIX . 'ac_user u ON n.u_id = u.u_id
				WHERE n_id = ? ';
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		{
			$myNotification = Core_Notification::factory($row['n_type']);
			...
			$myNotification->actor = new Core_User();
			$myNotification->actor->initMainInfo($row);
	...
	*/
	public static function factory($type)
	{
		...
		switch($type)
		{
			case self::TYPE_STATUS_ADD: $myNotification = new Core_Notification_StatusAdd(); break;
			case self::TYPE_FEED_COMMENT: $myNotification = new Core_Notification_FeedComment(); break;
			case self::TYPE_FEED_COMMENT_FOLLOW: $myNotification = new Core_Notification_FeedCommentFollow(); break;
			case self::TYPE_FEED_COMMENT_LIKE_FOLLOW: $myNotification = new Core_Notification_FeedCommentLikeFollow(); break;
			case self::TYPE_FEED_LIKE: $myNotification = new Core_Notification_FeedLike(); break;
			case self::TYPE_FEED_LIKE_COMMENT_FOLLOW: $myNotification = new Core_Notification_FeedLikeCommentFollow(); break;
			case self::TYPE_FEED_LIKE_FOLLOW: $myNotification = new Core_Notification_FeedLikeFollow(); break;
			case self::TYPE_FRIENDREQUEST_ACCEPT: $myNotification = new Core_Notification_FriendRequestAccept(); break;
			case self::TYPE_BLOG_COMMENT: $myNotification = new Core_Notification_BlogComment(); break;
			case self::TYPE_BLOG_COMMENT_FOLLOW: $myNotification = new Core_Notification_BlogCommentFollow(); break;
			case self::TYPE_SYSTEM_NOTIFY: $myNotification = new Core_Notification_SystemNotify(); break;

			case self::TYPE_MENTION_STATUS: $myNotification = new Core_Notification_MentionStatus(); break;
			case self::TYPE_MENTION_STATUSLINK: $myNotification = new Core_Notification_MentionStatusLink(); break;
			case self::TYPE_MENTION_FEEDCOMMENT: $myNotification = new Core_Notification_MentionFeedComment(); break;

			default: $myNotification = new Core_Notification();
		}
	...
	*/
	public static function cacheGetNotificationList($userid, &$cacheSuccess = false, $forceStore = false, $formData = array(), $limit = 10)
	{
		...
		//get current cache
		$notificationListRawData = apc_fetch($cacheKeystring);
	...
	*/
	public static function cacheSetNotificationList($userid, $value)
	{
		...
		return apc_store(self::cacheBuildKeystringNotificationList($userid), $value, $registry->setting['apc']['shortDelay']);
	}
	...
	*/
	...
	{
		return apc_delete(self::cacheBuildKeystringNotificationList($userid));
	}
