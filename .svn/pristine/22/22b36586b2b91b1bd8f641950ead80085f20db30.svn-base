...
class dbSession
{
	...
	function dbSession($pdoDb, $gc_maxlifetime = '', $gc_probability = '', $gc_divisor = '')
	{
		...
		$moreUpdate = '';
		$controllerString = htmlspecialchars(trim($GLOBALS['controller_group'])) . '_' . htmlspecialchars(trim($GLOBALS['controller']));
		if(isset($setting['site']['sessionExcludeController']))
		{
			if(!in_array($controllerString, $setting['site']['sessionExcludeController']))
			{
				$moreUpdate = 's_controller = "'.$controllerString.'", s_action = "'.htmlspecialchars($GLOBALS['action']).'",';
			}
		}
	...
	function get_users_online()
	{
		$sql = 'SELECT COUNT(*)
				...
				WHERE s_dateexpired > ?
				';
		return $this->db->query($sql, array(time()))->fetchColumn(0);
	}
	...
	function read($session_id)
	{
		...
		$sql = 'SELECT s_data
				...
				WHERE s_id = ?
					';
		$session_data =  $this->db->query($sql, array($session_id))->fetchColumn(0);
	...
	function write($session_id, $session_data)
	{
		...
		$sql = 'SELECT COUNT(*)
				...
				WHERE s_id = ?
				LIMIT 1';
		$count = $this->db->query($sql, array($session_id))->fetchColumn(0);
	...
	/**
	 * Tra ve Session ID cua user nay (neu dang online)
	 */
	public static function getUserSession($userId)
	{
		global $db;

		$sql = 'SELECT s_id FROM ' .  TABLE_PREFIX . 'sess
				WHERE s_userid = ?';
		$stmt = $db->query($sql, array($userId));

		$sessionList = array();
		while($row = $stmt->fetch())
		{
			$sessionList[] = $row['s_id'];
		}
