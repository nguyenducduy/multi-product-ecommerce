...
Class Core_Backend_ModeratorLog extends Core_Backend_Object
{
	public $uid = 0;
	public $email = '';
	public $id = 0;
	...
	public static function getLogs($formData, $sortby, $sorttype, $limit = '', $countOnly = false)
	{
		...
		if(strlen($formData['ftype']) > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'ml.ml_type LIKE \'%'.Helper::unspecialtext($formData['ftype']).'%\' ';

		if($formData['fmainid'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'ml.ml_mainid = '.(int)$formData['fmainid'].' ';
