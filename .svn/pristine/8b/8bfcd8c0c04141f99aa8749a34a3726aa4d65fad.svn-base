		...
		<tbody>
			...
			<tr>
				<td rowspan="3">
					...
					<span class="label label-info">{$parent.sc_name}</span> &raquo;
					{/foreach}
					{/if}
			...
			<tr>
				...
				<td><input type="checkbox" {foreach item=item key=scid from=$formData.fstuffcategory}{if $stuffcategory->id == $scid && $item == 'change'}checked="checked"{/if}{/foreach} value="change" name="fstuffcategory[{$stuffcategory->id}]" id="cg_{$stuffcategory->id}" onclick="clickfunc('cg_{$stuffcategory->id}' , 'cv_{$stuffcategory->id}')"/></td>
			</tr>
			<tr>
			</tr>
			{/foreach}
