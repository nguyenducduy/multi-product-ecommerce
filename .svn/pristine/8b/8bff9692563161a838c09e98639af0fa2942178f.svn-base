...
$(function () {		
    ...
    if($('#chartdata').length)
    {
        ...
        if(chartby == 'volume')
        {
            yAxisData.push({title:{text: yAxisLabel} , startOnTick: false , labels: {enabled: true,formatter: function() {return numeral(this.value).format('0,0');}},plotLines: [{value: 0,width: 1,color: '#808080'}] , lineColor: '#FF0000',
				lineWidth: 1,});
        }
        else
        {           
           yAxisData.push({title:{text: yAxisLabel} , startOnTick: false , tickPixelInterval : 40, labels: {enabled: true,formatter: function() {return numeral(this.value).format('0,0');}},plotLines: [{value: 0,width: 1,color: '#808080'}] , lineColor: '#FF0000',
				lineWidth: 1,});
        }
