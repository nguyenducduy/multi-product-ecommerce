<ul class="breadcrumb">
	...
	<li><a href="{$conf.rooturl_admin}{$controller}">Review Queue</a> <span class="divider">/</span></li>
	<li class="active">Listing</li>
</ul>

<div class="page-header" rel="menu_reviewqueue_list"><h1>{$lang.controller.head_list}</h1></div>
			...
			<form class="form-inline" action="" method="post" onsubmit="return confirm('Are You Sure ?');">
				...
				{foreach item=review from=$reviews}
					<tr id="review-{$review->id}">
						...
						<td style="font-weight:bold;"><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl_admin}reviewqueue/edit/id/{$review->id}/redirect/{$redirectUrl}">{$review->id}</a></td>
                        ...
                        <td class="td_left"><strong><a href="{$conf.rooturl_admin}reviewqueue/index/userid/{$review->actor->id}" title="View all reviews queue of this user">{$review->actor->fullname}</a></strong><br /><small><a class="ip2location" target="_blank" href="http://www.ip2location.com/{$review->ipaddress}" title="Trace this IP Address">{$review->ipaddress}</a></small></td>
						<td><strong><a href="{$conf.rooturl_admin}reviewqueue/index/bookid/{$review->book->id}" title="View all reviews queue of this book">{$review->book->title}</a></strong><br />
                        	<small>{$review->text|strip_tags}</small>
							<br />
							<a class="btn btn-success btn-mini btnaccept" title="Accept &amp; create REP" href="javascript:void(0)" onclick="reviewqueueaccept('{$conf.rooturl_admin}reviewqueue/acceptajax/id/{$review->id}', {$review->id});">Accept &amp; Add REP</a>
                        </td>
						...
						<td><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl_admin}reviewqueue/edit/id/{$review->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i> {$lang.default.formEditLabel}</a> &nbsp;
							<a title="{$lang.default.formActionDeleteTooltip}" href="javascript:delm('{$conf.rooturl_admin}reviewqueue/delete/id/{$review->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i> {$lang.default.formDeleteLabel}</a>
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl_admin + "reviewqueue/index";
	...
	function reviewqueueaccept(url, reviewid)
	{
		$("#review-" + reviewid + " .btnaccept").after('<img class="tmp_indicator" src="'+imageDir+'ajax_indicator.gif" border="0" />');
