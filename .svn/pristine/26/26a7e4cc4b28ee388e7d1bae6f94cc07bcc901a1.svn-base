...
Class Controller_Admin_Log Extends Controller_Admin_Base 
{
	...
	function indexAction() 
	{
		...
		$mainidFilter 	= (int)($this->registry->router->getArg('mainid'));
			...
			else
			{
				...
				if($_POST['fbulkaction'] == 'delete')
				{
					...
					foreach($delArr as $id)
					{
						...
						$myLog = new Core_Backend_ModeratorLog($id);
		...
		if($mainidFilter > 0)
		{
			$paginateUrl .= 'mainid/'.$mainidFilter . '/';
			$formData['fmainid'] = $mainidFilter;
			$formData['search'] = 'mainid';
		}
		...
		//tim tong so record
		$total = Core_Backend_ModeratorLog::getLogs($formData, $sortby, $sorttype, 0, true);
		$totalPage = ceil($total/$this->recordPerPage);
		$curPage = $page;
		
			
		//get latest records
		$logsTmp = Core_Backend_ModeratorLog::getLogs($formData, $sortby, $sorttype, (($page - 1)*$this->recordPerPage).','.$this->recordPerPage);
		$logs = array();
