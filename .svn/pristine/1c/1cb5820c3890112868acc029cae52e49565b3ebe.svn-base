...
*/
Class Core_NewsletterTask extends Core_Object
{
	public $nid = 0;
	public $id = 0;
	public $sendcount = 0;
    ...
    public static function deleteFromNewsletter($newsletterId)
    {
        ...
        $sql = 'DELETE FROM ' . TABLE_PREFIX . 'newsletter_task
                WHERE n_id = ?';
        $rowCount = $db->query($sql, array($newsletterId))->rowCount();
    ...
    public static function setSentStatusFromNewsletter($newsletterId, $status = 0)
    {
        ...
        $sql = 'UPDATE ' . TABLE_PREFIX . 'newsletter_task
                SET nt_issent = ?
                WHERE n_id = ?';
        $rowCount = $db->query($sql, array($status, $newsletterId))->rowCount();
        return $rowCount;
    }
    ...
    public static function getList($where, $order, $limit = '')
    {
        ...
        {
            $myNewsletterTask = new Core_NewsletterTask();
