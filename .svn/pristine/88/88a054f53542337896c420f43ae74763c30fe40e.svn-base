...
<div class="page-header" rel="menu_newscategory"><h1>{$lang.controller.head_edit}</h1></div>
...
<input type="hidden" name="ftoken" value="{$smarty.session.newscategoryEditToken}" />
    ...
    <div class="control-group">
        ...
        <div class="controls">
            <select name="fparentid">
                ...
                {foreach item=newscategorys from=$category}
                    <option value="{$newscategorys->id}"{if $newscategorys->id eq $formData.fparentid}selected="selected"{/if}>{$newscategorys->name}</option>
                {/foreach}
            </select>
        </div>
    </div>   
		...
		<div class="controls"><input type="text" name="fslug" id="fslug" value="{$formData.fslug|@htmlspecialchars}" class="input-xxlarge">
			{if $slugList|@count > 0}
				...
				{foreach item=slug from=$slugList}
					{if $slug->controller != 'newscategory' || $slug->objectid != $formData.fid}<div class="red">&raquo; {$slug->controller} / {$slug->objectid} <a href="{$slug->getSlugSearch()}" title="Go to this slug" class="tipsy-trigger"><i class="icon-share"></i></a></div>{/if}
				{/foreach}
			{/if}
		</div>
	</div>
	...
	<div class="control-group">
        ...
        Còn lại <span id="seodescriptioncounter">50</span> ký tự
        </div>
	</div>
    ...
    <div class="control-group">
        <label class="control-label" for="fimage"><img src="{$conf.rooturl}uploads/newscategory/{$formData.fimage}" /></label>
        <div class="controls">     
