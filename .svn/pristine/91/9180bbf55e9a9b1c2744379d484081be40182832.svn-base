<ul class="breadcrumb">
	...
	<li><a href="{$conf.rooturl}{$controllerGroup}/{$controller}">Bình luận rao vặt</a> <span class="divider">/</span></li>
	<li class="active">Listing</li>
</ul>



<div class="page-header" rel="menu_stuffreview"><h1>{$lang.controller.head_list}</h1></div>
	...
	<div class="tab-content">
		<div class="tab-pane active" id="tab1">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				<input type="hidden" name="ftoken" value="{$smarty.session.stuffreviewBulkToken}" />
				<table class="table table-striped">
		
				{if $stuffreviews|@count > 0}
					<thead>
						<tr>
					...
					<tbody>
					{foreach item=stuffreview from=$stuffreviews}
		
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$stuffreview->id}" {if in_array($stuffreview->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td><span class="badge">{$stuffreview->id}</span></td>
							<td>{$stuffreview->parentid}</td>
							<td>{$stuffreview->datecreated|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td><span class="label label-info">{if $stuffreview->actor->id > 0}{$stuffreview->actor->fullname}{else}{$stuffreview->fullname}{/if}</span></td>
							<td><a target="_blank" href="{$conf.rooturl_cms}stuff/index/id/{$stuffreview->stuff->id}">{$stuffreview->stuff->title}</a></td>
							<td>{$stuffreview->email}</td>
							<td>{$stuffreview->text}</td>
							<td><span class="badge badge-info">{$stuffreview->countreply}<span></td>
							<td>
								{if $stuffreview->checkStatusName('enable')}
									<span class="label label-success">{$stuffreview->getStatusName()}</span>
								{elseif $stuffreview->checkStatusName('pending')}
									<span class="label label-warning">{$stuffreview->getStatusName()}</span>
								{else}
									<span class="label label-important">{$stuffreview->getStatusName()}</span>
								{/if}
							</td>
							
							<td>
								<span class="tipsy-trigger" title="{$lang.default.formEditLabel}"><a href="{$conf.rooturl}{$controllerGroup}/{$controller}/edit/id/{$stuffreview->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i></a></span> &nbsp;
								
								<span class="tipsy-trigger" title="{$lang.default.formDeleteLabel}"><a href="javascript:delm('{$conf.rooturl}{$controllerGroup}/{$controller}/delete/id/{$stuffreview->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i></a></span> &nbsp;

								<a href="{$conf.rooturl_cms}stuffreview/reply/parentid/{$stuffreview->id}" rel="shadowbox;height=195px;width=800px" class="btn btn-mini"><i class="icon-white"></i>Reply</a>
							</td>
						</tr>
		...
		<div class="tab-pane" id="tab3">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				...
				{if $totalpending > 0}
					...
					<tbody>
					{foreach item=stuffreview from=$stuffreviews}
						{if $stuffreview->checkStatusName('pending')}
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$stuffreview->id}" {if in_array($stuffreview->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td><span class="badge">{$stuffreview->id}</span></td>
							<td>{if $stuffreview->parentid > 0}{$stuffreview->parentid}{/if}</td>
							<td>{$stuffreview->datecreated|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td><span class="label label-info">{if $stuffreview->actor->id > 0}{$stuffreview->actor->fullname}{else}{$stuffreview->fullname}{/if}</span></td>
							<td><a target="_blank" href="{$conf.rooturl_cms}stuff/index/id/{$stuffreview->stuff->id}">{$stuffreview->stuff->title}</a></td>
							<td>{$stuffreview->email}</td>
							<td>{$stuffreview->text}</td>
							<td><span class="badge badge-info">{$stuffreview->countreply}<span></td>
							<td>{if $stuffreview->checkStatusName('enable')}
									<span class="label label-success">{$stuffreview->getStatusName()}</span>
								{elseif $stuffreview->checkStatusName('pending')}
									<span class="label label-warning">{$stuffreview->getStatusName()}</span>
								{else}
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl + controllerGroup + "/stuffreview/index";
