...
<div id="panelleft">
		...
		{include file="`$smartyControllerGroupContainer`leftheading-top.tpl"}
			...
			<div class="breadcrumbs">
				<span>{$lang.controller.titleEdit}</span> &laquo; <a href="{$me->getUserPath()}/blog" title="{$lang.controller.titleAlt}">{$lang.controller.titleAlt}</a>
			</div>
	...
	{include file="notify.tpl" notifyError=$error}
		<div id="blogform" class="myform myformwide stylizedform">
			<form id="form1" name="form1" method="post" action="" enctype="multipart/form-data">
				<h1>{$lang.controller.titleEdit}</h1>
				<p></p>
				...
				{if $myBlog->image != ''}
					<label>&nbsp;</label>
					<a href="{$myBlog->getImage()}" target="_blank"><img src="{$myBlog->getImage()}" border="0" width="150" /></a><br />
					<div class="clear"></div>
					<label>&nbsp;</label>
					<input class="input_inline" style="float:none;" type="checkbox" name="fdeleteimage" value="1" />{$lang.controller.deleteImage}<br /><br />
					<div class="clear"></div>
				{/if}
			...
			<div id="sharemodewrapper">
				...
				<div class="text textwide">
					...
					{if $tagbookList|@count > 0}
						...
						<option value="{$tagbook->id}" class="selected">{$tagbook->title|truncate:50}</option>
						{/foreach}
					{/if}
					</select>
				</div>
				...
				<input type="submit" class="submit" name="fsubmit" value="{$lang.controller.save}"  />
