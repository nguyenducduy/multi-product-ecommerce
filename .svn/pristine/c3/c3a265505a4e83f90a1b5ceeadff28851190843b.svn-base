<ul class="breadcrumb">
	...
	<li><a href="{$conf.rooturl}{$controllerGroup}/{$controller}">Bình luận tin</a> <span class="divider">/</span></li>
	<li class="active">Listing</li>
</ul>



<div class="page-header" rel="menu_newsreview"><h1>{$lang.controller.head_list}</h1></div>
	...
	<div class="tab-content">
		<div class="tab-pane active" id="tab1">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				<input type="hidden" name="ftoken" value="{$smarty.session.newsreviewBulkToken}" />
				<table class="table table-striped">
		
				{if $newsreviews|@count > 0}
					<thead>
						<tr>
					...
					<tbody>
					{foreach item=newsreview from=$newsreviews}
		
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$newsreview->id}" {if in_array($newsreview->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td><span class="badge">{$newsreview->id}</span></td>
							<td>{if $newsreview->parentid > 0}{$newsreview->parentid}{/if}</td>
							<td>{$newsreview->datemoderated|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td><span class="label label-info">{if $newsreview->actor->id > 0}{$newsreview->actor->fullname}{else}{$newsreview->fullname}{/if}</span></td>
							<td><a target="_blank" href="{$conf.rooturl_cms}news/index/id/{$newsreview->news->id}">{$newsreview->news->title}</a></td>
							<td>{$newsreview->email}</td>
							<td>{$newsreview->text}</td>
							<td><span class="badge badge-info">{$newsreview->countreply}</span></td>
							<td>
								{if $newsreview->checkStatusName('enable')}
									<span class="label label-success">{$newsreview->getStatusName()}</span>
								{elseif $newsreview->checkStatusName('pending')}
									<span class="label label-warning">{$newsreview->getStatusName()}</span>
								{else}
									<span class="label label-important">{$newsreview->getStatusName()}</span>
								{/if}
							</td>
							<td>
								<span class="tipsy-trigger" title="{$lang.default.formActionEditTooltip}"><a href="{$conf.rooturl}{$controllerGroup}/{$controller}/edit/id/{$newsreview->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i></a> </span>&nbsp;

								<span class="tipsy-trigger" title="{$lang.default.formActionDeleteTooltip}"><a href="javascript:delm('{$conf.rooturl}{$controllerGroup}/{$controller}/delete/id/{$newsreview->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i></a></span> &nbsp;

								<a href="{$conf.rooturl_cms}newsreview/reply/parentid/{$newsreview->id}" rel="shadowbox;height=195px;width=800px" class="btn btn-mini"><i class="icon-white"></i>Reply</a>
							</td>
						</tr>
		...
		<div class="tab-pane" id="tab3">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				...
				{if $totalpending > 0}
					...
					<tbody>
					{foreach item=newsreview from=$newsreviews}
						{if $newsreview->checkStatusName('pending')}
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$newsreview->id}" {if in_array($newsreview->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td><span class="badge">{$newsreview->id}</span></td>
							<td>{if $newsreview->parentid > 0}{$newsreview->parentid}{/if}</td>
							<td>{$newsreview->datecreated|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td><span class="label label-info">{if $newsreview->actor->id > 0}{$newsreview->actor->fullname}{else}{$newsreview->fullname}{/if}</span></td>
							<td><a target="_blank" href="{$conf.rooturl_cms}news/index/id/{$newsreview->news->id}">{$newsreview->news->title}</a></td>
							<td>{$newsreview->email}</td>
							<td>{$newsreview->text}</td>
							<td><span class="badge badge-info">{$newsreview->countreply}</span></td>
							<td>{if $newsreview->checkStatusName('enable')}
									<span class="label label-success">{$newsreview->getStatusName()}</span>
								{elseif $newsreview->checkStatusName('pending')}
									<span class="label label-warning">{$newsreview->getStatusName()}</span>
								{else}
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl + controllerGroup + "/newsreview/index";
