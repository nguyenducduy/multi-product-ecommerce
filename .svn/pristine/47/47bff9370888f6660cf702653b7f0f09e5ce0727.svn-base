...
Class Core_View extends Core_Object
{
    ...
    public function checkTypeName($name)
    {
        ...
        if(
			($this->type == self::TYPE_GROUPCONTROLLER_SITE && $name == 'groupcontrollersite' )
			|| ($this->type == self::TYPE_HOMEPAGE && $name == 'homepage' )
			|| ($this->type == self::TYPE_PRODUCT && $name == 'product' )
			|| ($this->type == self::TYPE_SEARCH && $name == 'search' )
			|| ($this->type == self::TYPE_PRODUCTCATEGORY && $name == 'productcategory' )
			|| ($this->type == self::TYPE_NEWS && $name == 'news' )
			|| ($this->type == self::TYPE_NEWSCATEGORY && $name == 'newscategory' )
			|| ($this->type == self::TYPE_STUFF && $name == 'stuff')
			|| ($this->type == self::TYPE_STUFFCATEGORY && $name == 'stuffcategory')
			|| ($this->type == self::TYPE_PAGE && $name == 'page')
			)
            return true;
        else
            return false;
