function profileadvanced_addwork()
{
	...
	else
	{
		...
		$.ajax({
		   ...
		   url: rooturl_profile + 'account/additem',
		   error: function(){
...
function profileadvanced_editwork(pid)
{
	...
	else
	{
		...
		$.ajax({
		   ...
		   dataType: 'xml',
		   data: 'type=work&text1=' + encodeURIComponent(company) + '&text2=' + encodeURIComponent(jobtitle) + '&text3=' + region + '&fromyear=' + fromyear + '&frommonth=' + frommonth + '&toyear=' + toyear + '&tomonth=' + tomonth + '&tonow=' + tonow,
		   url: rooturl_profile + 'account/edititem?id=' + pid,
		   error: function(){
	...
	bootbox.confirm(delConfirm, function(confirmed){
		if(confirmed)
		{
			...
			$.ajax({
			   type: "GET",
			   dataType: 'xml',
			   url: rooturl_profile + 'account/deleteitem?id=' + pid,
			   error: function(){
...
function profileadvanced_addotherfield()
{
	...
	{
		showGritterError('Bạn chưa nhập tên thông tin');
		...
		$.ajax({
		   ...
		   success: function(xml){
				...
				if(success == "1")
				{
					...
					var html = '<div id="profile-'+pid+'" class="profileitem profileitemnoborder hide control-group">';
					html += '<div class="profileitembutton"><a class="profiledelete" href="javascript:void(0)" onclick="profileadvanced_delete('+pid+')">Xóa</a></div>';
					html += '<label class="control-label" for="fieldname">'+fieldname+'</label>';
					html += '<div class="controls"><input type="text" readonly="readonly" class="fieldvaluetext profileiteminputdisabled" value="'+fieldvalue+'" /></div>';
					html += '</div>';
