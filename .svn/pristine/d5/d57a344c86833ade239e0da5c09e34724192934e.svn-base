...
<div class="page-header" rel="menu_news"><h1>{$lang.controller.head_edit}</h1></div>
...
<input type="hidden" name="ftoken" value="{$smarty.session.newsEditToken}" />
		...
		<div class="controls">
            <select name="fncid">
                ...
                {foreach item=category from=$newscategorys}
                    ...
                    <option value="{$category->id}"{if $category->id == $formData.fncid}selected="selected"{/if}disabled style="background-color: whiteSmoke;">{$category->name}</option>
                    {else}
		...
		<div class="controls"><input type="text" name="fslug" id="fslug" value="{$formData.fslug|@htmlspecialchars}" class="input-xxlarge">
			{if $slugList|@count > 0}
				...
				{foreach item=slug from=$slugList}
					{if $slug->controller != 'news' || $slug->objectid != $formData.fid}<div class="red">&raquo; {$slug->controller} / {$slug->objectid} <a href="{$slug->getSlugSearch()}" title="Go to this slug" class="tipsy-trigger"><i class="icon-share"></i></a></div>{/if}
				{/foreach}
			{/if}
		</div>
	</div>
	...
	<div class="control-group">
		...
		<div class="controls"><textarea name="fcontent" id="fcontent" rows="25" class="input-xxlarge">{$formData.fcontent}</textarea></div>
    ...
    <div class="control-group">
        <label class="control-label" for="fimage">{if $formData.fimage != ''}<img src="{$conf.rooturl}uploads/news/{$formData.fimage}"/>{/if}</label>       
        <div class="controls">          
