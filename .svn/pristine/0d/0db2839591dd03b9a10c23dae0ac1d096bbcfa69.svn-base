<ul class="breadcrumb">
	...
	<li><a href="{$conf.rooturl}{$controllerGroup}/{$controller}">StuffReviewThumb</a> <span class="divider">/</span></li>
	<li class="active">Listing</li>
</ul>



<div class="page-header" rel="menu_stuffreviewthumb"><h1>{$lang.controller.head_list}</h1></div>
	...
	<div class="tab-content">
		<div class="tab-pane active" id="tab1">
			...
			<form action="" method="post" name="manage" class="form-inline" onsubmit="return confirm('Are You Sure ?');">
				<input type="hidden" name="ftoken" value="{$smarty.session.stuffreviewthumbBulkToken}" />
				<table class="table table-striped">
		
				{if $stuffreviewthumbs|@count > 0}
					<thead>
						<tr>
						   <th width="40"><input class="check-all" type="checkbox" /></th>
							...
							<th><a href="{$filterUrl}sortby/value/sorttype/{if $formData.sortby eq 'value'}{if $formData.sorttype|upper neq 'DESC'}DESC{else}ASC{/if}{/if}">{$lang.controller.labelValue}</a></th>
					...
					<tbody>
					{foreach item=stuffreviewthumb from=$stuffreviewthumbs}
		
						<tr>
							<td><input type="checkbox" name="fbulkid[]" value="{$stuffreviewthumb->id}" {if in_array($stuffreviewthumb->id, $formData.fbulkid)}checked="checked"{/if}/></td>
							<td style="font-weight:bold;">{$stuffreviewthumb->id}</td>
							
							<td>{$stuffreviewthumb->robjectid}</td>
							<td>{$stuffreviewthumb->rid}</td>
							<td>{$stuffreviewthumb->uid}</td>
							<td>{$stuffreviewthumb->value}</td>
							<td>{$stuffreviewthumb->ipaddress|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td>{$stuffreviewthumb->datecreated|date_format:$lang.default.dateFormatTimeSmarty}</td>
							<td>{$stuffreviewthumb->datemodified|date_format:$lang.default.dateFormatTimeSmarty}</td>
							
							<td><a title="{$lang.default.formActionEditTooltip}" href="{$conf.rooturl}{$controllerGroup}/{$controller}/edit/id/{$stuffreviewthumb->id}/redirect/{$redirectUrl}" class="btn btn-mini"><i class="icon-pencil"></i> {$lang.default.formEditLabel}</a> &nbsp;
								<a title="{$lang.default.formActionDeleteTooltip}" href="javascript:delm('{$conf.rooturl}{$controllerGroup}/{$controller}/delete/id/{$stuffreviewthumb->id}/redirect/{$redirectUrl}?token={$smarty.session.securityToken}');" class="btn btn-mini btn-danger"><i class="icon-remove icon-white"></i> {$lang.default.formDeleteLabel}</a>
							</td>
						</tr>
...
<script type="text/javascript">
	function gosearch()
	{
		var path = rooturl + controllerGroup + "/stuffreviewthumb/index";
