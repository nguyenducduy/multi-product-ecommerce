<div class="specialproductsegment">
	{if !empty($productDetail)}			            	
		...
		<p class="productsegmentbuybutton">
			<a class="productsegmentbuylink" href="{$conf.rooturl}cart/{if $productDetail->slug != ""}mua-{$productDetail->slug}{else}checkout?id={$productDetail->id}{/if}{if $promoid > 0}{if $productDetail->slug != ""}?{else}&{/if}prid={$productDetail->barcode|trim}_{$promoid}{/if}{if $programid != ''}{if $promoid > 0 && $productDetail->slug != ""}&{else}?{/if}po=p{substr($productDetail->barcode|trim, -5, 5)}{$programid}{/if}">Mua ngay</a>
		</p>
