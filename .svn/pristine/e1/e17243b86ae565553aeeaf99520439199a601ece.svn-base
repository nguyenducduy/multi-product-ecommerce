...
Class Controller_Cms_Language Extends Controller_Cms_Base  
{
	...
	function indexAction()
	{
									...
									if($dh3 = opendir($workingLanguageDir.$langdir.DIRECTORY_SEPARATOR.$file))
									{
										while(($controllerfile = readdir($dh3)) !== false)
										{
											if(strtolower(substr($controllerfile,-3)) == 'xml')
											{
												...
												$searchList[] = $langdir . '/' . $file . '/' . $controllerfile;
											}
										}
		...
		if($keyword != '')
		{
			//tien hanh loop tat ca file de search
			foreach($searchList as $file)
			{
				$filecontents = file_get_contents($workingLanguageDir . $file);
				if(stripos($filecontents, $keyword) !== false)
				{
					$fileinfo = array('folder' => '', 'subfolder' => '', 'file' => '', 'fullpath' => $file);
					$fileparts = explode('/', $file);
					
					$fileinfo['folder'] = $fileparts[0];
					
					if(count($fileparts) == 2)
					{
						$fileinfo['file'] = $fileparts[1]; 
					}
					else
					{
						$fileinfo['subfolder'] = $fileparts[1];
						$fileinfo['file'] = $fileparts[2];
					}
			...
			{
				$error[] = 'Can not find Language file contents your search string <em>'.$keyword.'</em>';
			}
		}
