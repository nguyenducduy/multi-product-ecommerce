...
/**
 * core/class.pagereview.php
 *
 * File contains the class used for PageReview Model
 *
 * @category Litpi
 * @package Core
 ...
 */
Class Core_PageReview extends Core_Object
{
    ...
    public function addData()
    {
		...
		$sql = 'INSERT INTO ' . TABLE_PREFIX . 'page_review (
					u_id,
					r_objectid,
					r_fullname,
					r_email,
	...
	public function updateData()
	{
		...
		$sql = 'UPDATE ' . TABLE_PREFIX . 'page_review
				SET u_id = ?,
					r_objectid = ?,
					r_fullname = ?,
					r_email = ?,
	...
	{
		$id = (int)$id;
		$sql = 'SELECT * FROM ' . TABLE_PREFIX . 'page_review pr
				WHERE pr.r_id = ?';
	...
	{
		$sql = 'DELETE FROM ' . TABLE_PREFIX . 'page_review
				WHERE r_id = ?';
	...
	public static function countList($where)
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . TABLE_PREFIX . 'page_review pr';
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		while($row = $stmt->fetch())
		{
			...
			$myPageReview->uid = $row['u_id'];
			$myPageReview->id = $row['r_id'];
			$myPageReview->objectid = $row['r_objectid'];
			$myPageReview->fullname = $row['r_fullname'];
			$myPageReview->email = $row['r_email'];
			$myPageReview->text = $row['r_text'];
			$myPageReview->ipaddress = $row['r_ipaddress'];
			$myPageReview->moderatorid = $row['r_moderatorid'];
			$myPageReview->status = $row['r_status'];
			$myPageReview->countthumbup = $row['r_countthumbup'];
			$myPageReview->countthumbdown = $row['r_countthumbdown'];
			$myPageReview->countreply = $row['r_countreply'];
			$myPageReview->datecreated = $row['r_datecreated'];
			$myPageReview->datemodified = $row['r_datemodified'];
			$myPageReview->datemoderated = $row['r_datemoderated'];
			$myPageReview->parentid = $row['r_parentid'];


            $outputList[] = $myPageReview;
        }
	 ...
	 *
	 */
	public static function getPageReviews($formData, $sortby, $sorttype, $limit = '', $countOnly = false)
	{
		$whereString = '';
		...
		if($formData['fipaddress'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'pr.r_ipaddress = '.(int)$formData['fipaddress'].' ';
		...
		if($formData['fdatecreated'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'pr.r_datecreated = '.(int)$formData['fdatecreated'].' ';
		...
		if(strlen($formData['fkeywordFilter']) > 0)
		{
			...
			elseif($formData['fsearchKeywordIn'] == 'text')
				$whereString .= ($whereString != '' ? ' AND ' : '') . 'pr.r_text LIKE \'%'.$formData['fkeywordFilter'].'%\'';
			else
				$whereString .= ($whereString != '' ? ' AND ' : '') . '( (pr.r_fullname LIKE \'%'.$formData['fkeywordFilter'].'%\') OR (pr.r_email LIKE \'%'.$formData['fkeywordFilter'].'%\') OR (pr.r_text LIKE \'%'.$formData['fkeywordFilter'].'%\') )';
		}
	...
	public static function getFullReview($review)
	{
		...
		$output = array();
		$output[] = $review;
		$replyReviewList = Core_PageReview::getPageReviews(array('fobjectid' => $fpid,
                                                                        'fparentid' => $review->id,
                                                                        'fstatus' => Core_PageReview::STATUS_ENABLE) , 'id' , 'ASC');
