...
Class Core_Friend extends Core_Object
{
	...
	*/
	public static function deleteFollowing($userid, $followinguserid)
	{
		...
		$rowCount = $db->query($sql, array($userid, $followinguserid))->rowCount();
	...
	*/
	public static function addFollowing($userid, $followinguserid, $type = 0)
	{
		...
		$sql = 'INSERT IGNORE INTO ' . TABLE_PREFIX . 'friend(u_id, u_id_friend, f_type, f_datecreated)
				VALUES(?,?,?, ?)
				';
		$rowCount = $db->query($sql, array($userid, $followinguserid, $type, $timeadd))->rowCount();
	...
	*/
	public static function checkFollowing($userid, $followinguserid)
	{
		...
		else
		{
			$sql = 'SELECT COUNT(*) FROM ' . TABLE_PREFIX . 'friend
					WHERE u_id = ? AND u_id_friend = ?';
			$connection = $db->query($sql, array($userid, $followinguserid))->fetchColumn(0);
			$result = ($connection >= 1);
	...
	public static function getListFollower($where, $order, $limit = '')
	{
		...
		while($row = $stmt->fetch())
		{
			...
			$myFriend->actor = new Core_User();
			$myFriend->actor->initMainInfo($row);
			$outputList[] = $myFriend;
		}
		return $outputList;
	}
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		while($row = $stmt->fetch())
		{
			...
			$myFriend->actor = new Core_User();
			$myFriend->actor->initMainInfo(array_merge($row, array('u_id' => $row['u_id_friend'])));	//using friendid to init data for friend
			$outputList[] = $myFriend;
		}
		return $outputList;
	}
