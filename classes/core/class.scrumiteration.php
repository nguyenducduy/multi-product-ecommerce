 ...
 */
Class Core_ScrumIteration extends Core_Object
{
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		{
			$myScrumIteration = new Core_ScrumIteration();

			$myScrumIteration->spid = $row['sp_id'];
			$myScrumIteration->stid = $row['st_id'];
			$myScrumIteration->ssspid = $row['sss_id'];
	...
	public static function getScrumIterations($formData, $sortby, $sorttype, $limit = '', $countOnly = false)
	{
		...
		if($formData['fsssid'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 'si.sss_id = '.(int)$formData['fsssid'].' ';
