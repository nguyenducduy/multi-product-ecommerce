 ...
 */
Class Core_Productindex extends Core_Object
{
    ...
    public function updateData()
    {
        ...
        if($myProduct->id > 0 && $myProduct->status == Core_Product::STATUS_ENABLE && $myProduct->onsitestatus >0 && (($myProduct->sellprice > 0 && $myProduct->instock > 0) || ($myProduct->prepaidprice > 0 && $myProduct->prepaidstartdate <= $currentTime && $myProduct->prepaidenddate >= $currentTime) || $currentTime->onsitestatus == Core_Product::OS_COMMINGSOON) )
        {
			$this->productstatus = 1;
        }
        else $this->productstatus = 0;
        ...
        $myProductIndex = new Core_Productindex($myProduct->id);
        $stmt = false;
    ...
    public function getData($pid)
    {
        ...
        $this->productstatus = $row['pi_productstatus'];
    ...
    public function delete()
    {
        $sql = 'DELETE FROM ' . TABLE_PREFIX . 'productindex
                WHERE  = ?';
        $rowCount = $this->db->query($sql, array($this->pid))->rowCount();
    ...
    public static function getList($where, $order, $limit = '')
    {
        ...
        {
            $myProductindex = new Core_Productindex();
            ...
            $myProductindex->productstatus = $row['pi_productstatus'];
	...
	public static function fetchall()
	{
        ...
        $totalRecord = $db->query('SELECT count(*) FROM '. TABLE_PREFIX . 'product')->fetchColumn(0);
        $recordperpage = 500;
        ...
        for($i = 0; $i < $totalPage; $i++ )
        {
            ...
            $sql = 'SELECT p_id FROM ' . TABLE_PREFIX . 'product
                    WHERE p_customizetype = ?
                ORDER BY p_id ASC LIMIT ' . $offset . ', ' . $recordperpage;

            $stmt = $db->query($sql , array(Core_Product::CUSTOMIZETYPE_MAIN));
            while($row = $stmt->fetch())
            {
                $myProductindex = new Core_Productindex();
                $myProductindex->pid = $row['p_id'];
