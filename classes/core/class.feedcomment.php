...
Class Core_FeedComment extends Core_Object
{
	public $uid = 0;
	public $fid = 0;
	public $id = 0;
	...
	public function delete($feedId = 0)
	{
		if($feedId > 0)
		{
			$sql = 'DELETE FROM ' . TABLE_PREFIX . 'feed_comment
        			WHERE f_id =  ? ';
			$rowCount = $this->db->query($sql, array((int)$feedId))->rowCount();
		}
		else
		{
			$sql = 'DELETE FROM ' . TABLE_PREFIX . 'feed_comment
	...
	* 
	* Thuong duoc goi sau khi xoa 1 feed tu core_feed
	* 
	* @param int $feedId
	*/
	...
	private function getData($id)
	{
		$sql = 'SELECT * FROM ' . TABLE_PREFIX . 'feed_comment fc
				INNER JOIN ' . TABLE_PREFIX . 'ac_user u ON fc.u_id = u.u_id
				WHERE fc_id = ? ';
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		{
			$myFeedComment = new Core_FeedComment();
			...
			$myFeedComment->actor = new Core_User();
			$myFeedComment->actor->initMainInfo($row);
