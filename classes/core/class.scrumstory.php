 ...
 */
Class Core_ScrumStory extends Core_Object
{
	const STATUS_TODO = 1;
    ...
    public static function deleteByproject($id)
    {
		...
		Core_ScrumStoryComment::deleteByStoryDelete($id);
		Core_ScrumStoryAsignee::deleteByStoryDelete($id);
		return $rowCount;
    }
    /**
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		{
			$myScrumStory = new Core_ScrumStory();
   ...
   public static function getScrumStorysJoinIterationCategory($formData, $sortby, $sorttype)
    {
        ...
        if($formData['fcategoryid'] > 0)
            $whereString .= ($whereString != '' ? ' AND ' : '') . 'ssc.ssc_name like "%'.Helper::plaintext((string)$formData['fcategoryid']).'" ';
...
//            $whereString .= ($whereString != '' ? ' AND ' : '') . 'ss.ss_categoryid = '.(int)$formData['fcategoryid'].' ';
    ...
    public static function countListFilterProjectCategory($sp_id, $ss_categoryid)
    {
        ...
        return Core_ScrumStory::countList($whereSS);
    }
