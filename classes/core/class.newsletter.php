...
Class Core_Newsletter extends Core_Object
{
	public $id = 0;
	public $uid = 0;
	public $fromemail = '';
    ...
    public function getData($id)
    {
        ...
        $sql = 'SELECT * FROM ' . TABLE_PREFIX . 'newsletter n
        		LEFT JOIN ' . TABLE_PREFIX . 'ac_user u ON n.u_id = u.u_id
                WHERE n.n_id = ?';
    ...
    public static function getList($where, $order, $limit = '')
    {
        ...
        {
            $myNewsletter = new Core_Newsletter();
	        ...
	        $myNewsletter->actor = new Core_User();
	        $myNewsletter->actor->initMainInfo($row);
	...
	public function updateLastSentUser()
	{
		$sql = 'UPDATE ' . TABLE_PREFIX . 'newsletter SET u_id = ?
	    		WHERE n_id = ?';
		$stmt = $this->db->query($sql, array($this->uid, $this->id));
		return $stmt;
	}
