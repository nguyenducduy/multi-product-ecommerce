 ...
 */
Class Core_File extends Core_Object
{
    ...
    public function addData()
    {
		...
		if($this->id > 0)
		{
			if(strlen($_FILES['ffile']['name']) > 0)
	        {
				...
				$myFileDrive = Core_FileDrive::getFromChecksum($checksum);



				if($myFileDrive->id == 0)
				{
					$myFileDrive = new Core_FileDrive();
					$myFileDrive->uid = $this->uid;
					$myFileDrive->status = Core_FileDrive::STATUS_ENABLE;
	...
	public static function getFullParentDirectories($fileId)
	{
		...
		$directoryList = $db->query($sql, array($myFile->parentid))->fetchAll();

		foreach($directoryList as $directory)
		{
			$myDirectory = new Core_File();
	...
	public static function getFullChildren($parentid, $prefix, $idOnly = false)
	{
		...
		$sql = 'SELECT * FROM ' . TABLE_PREFIX . 'file f
				WHERE f_parentid = ?';
		$filelist = $db->query($sql, array($parentid))->fetchAll();
