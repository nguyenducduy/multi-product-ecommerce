...
############################################################
class Core_CookieCart extends Core_Object
{
		...
		if (!empty($_COOKIE['cartSessionDetail']))
		{
			...
			if (!empty($listcartdetail))
			{
				foreach ($listcartdetail as $cartdetail)
				{
					...
					{
						$this->addItem($cartdetailexplode[0], $cartdetailexplode[1], array('promotionid' => (!empty($cartdetailexplode[2])?$cartdetailexplode[2]:''), 'regionid' => 3));//mac dinh region la ho chi minh
					}
				}
			}
		}
	...
	public function saveToSession()
	{
		...
		if (!empty($items))
		{
			...
			{
				$arrayitemsaved[] = $item->id.':'.$item->quantity.':'.(!empty($item->options['promotionid'])?$item->options['promotionid']:'');
				//array('p_id' => $item->id, 'cd_quantity' => $item->quantity, 'cd_attribute' => serialize($item->options));
			}
