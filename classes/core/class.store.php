 ...
 */
Class Core_Store extends Core_Object
{
	...
	{
		$sql = 'DELETE FROM ' . TABLE_PREFIX . 'store
				WHERE s_id = ?';
	...
	public static function countList($where)
	{
		...
		$sql = 'SELECT COUNT(*) FROM ' . TABLE_PREFIX . 'store s';
	 ...
	 *
	 */
	public static function getStores($formData, $sortby, $sorttype, $limit = '', $countOnly = false)
	{
		$whereString = '';
		...
		if($formData['fidarr'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 's.s_id IN( '.implode(',' , $formData['fidarr']).') ';

		if($formData['fstoregroupidarr'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 's.s_storegroupid IN( '.implode(',' , $formData['fstoregroupidarr']).') ';
		...
		if(isset($formData['fhoststoreid']))
		...
		if($formData['fisautostorechange'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 's.s_isautostorechange = '.(int)$formData['fisautostorechange'].' ';
		...
		if($formData['fisinputstore'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 's.s_isinputstore = '.(int)$formData['fisinputstore'].' ';

		if($formData['fprovinceid'] > 0)
			$whereString .= ($whereString != '' ? ' AND ' : '') . 's.s_provinceid = '.(int)$formData['fprovinceid'].' ';
		...
		if(isset($formData['specialstore']))
			$whereString .= ($whereString != '' ? ' OR ' : '') . '(s_id = 5 OR s_id = 891) ';
		...
		if(count($formData['withoutstore']) > 0)
			$whereString .= ($whereString != '' ? ' OR ' : '') . 's_id NOT IN('.  implode(',' , $formData['withoutstore']).') ';
	...
	*/
    ...
    public static function getStoresFromCache($getmainstore = true)
    {
    	global $conf;
    	$storelist = array();
    	...
    	if($data != false)
    	{
    		...
    		foreach ($datalist as $datarow)
    		{
    			if($getmainstore)
	    		{
	    			if($datarow['isautostorechange'] == 1 || $datarow['id'] == 5 || $datarow['id'] == 891)
	    			{
	    				$storelist[$datarow['id']] = $datarow['name'];
	    			}
	    		}
	    		else
	    		{
    	...
    	/*global $db;
    	$sql =  'SELECT s_id , s_storeshortname FROM ' . TABLE_PREFIX . 'store';

    	$stmt = $db->query($sql);

    	while ( $row =  $stmt->fetch())
    	{
    		$storelist[$row['s_id']] = $row['s_storeshortname'];
    	}*/

    	return $storelist;
    }

    public static function getGroupStoreFromCache($storeid)
    {
    	global $conf;
    	$groupstore = array();
    	if($storeid > 0)
    	{
	    	...
	    	{
	    		$datalist = json_decode($data , true);
	    		if(strlen($datalist[$storeid]['groupstore']) > 0)
	    		{
	    			$groupstore = explode(',', $datalist[$storeid]['groupstore']);
	    		}
	    	}
    	}
    ...
    public static function checkisMarket($sid)
    {
    	global $db;
    	$output = false;
    	$sql = 'SELECT * FROM  ' . TABLE_PREFIX .'store s
    					WHERE  s.s_hoststoreid = 0
    					AND  s.s_issalestore = 1
    					AND  s.s_isinputstore =1
						AND s.s_isautostorechange =1
						AND s.s_id = ?';
		$row = $db->query($sql,array($sid))->fetch();
		if(!empty($row))
		{
			$output = true;
		}
    ...
    public static function getMainMarket($storegroupid)
    {
    	global $db;
    	$output = '';
    	$sql = 'SELECT * FROM ' .TABLE_PREFIX. 'store
				WHERE  s_hoststoreid =0
				AND  s_issalestore =1
				AND  s_isinputstore =1
				AND s_isautostorechange =1
				AND s_storegroupid = ?';
		$row = $db->query($sql,array($storegroupid))->fetch();
