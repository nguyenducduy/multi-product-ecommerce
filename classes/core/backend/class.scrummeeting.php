...
/**
 * core/backend/class.scrummeeting.php
 *
 ...
 */
Class Core_Backend_ScrumMeeting extends Core_Backend_Object
{
	...
	public static function joiniteration($id)
	{
		$output = '';
		$db3 = Core_Backend_Object::getDb();
		$sql = "select b.sp_id from lit_scrum_meeting a inner join lit_scrum_iteration b on b.si_id = a.si_id where a.sm_id = '".$id."' ";
		$tmp = $db3->query($sql);
		while($row = $tmp->fetch())
		{
			$output = $row['sp_id'];
