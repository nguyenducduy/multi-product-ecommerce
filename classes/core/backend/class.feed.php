...
Class Core_Backend_Feed extends Core_Backend_Object
{
	...
	*/
	public function delete()
	{
		...
		{
			//remove feedcomment
			Core_Backend_FeedComment::deleteFromFeed($this->id);
			
			//remove feedlike
			Core_Backend_FeedLike::deleteFromFeed($this->id);
		}
	...
	private function getData($id)
	{
		...
		$this->receiver = new Core_User($row['u_id_receive'], true);
	...
	public static function getList($where, $order, $limit = '')
	{
		...
		{
			$myFeed = Core_Backend_Feed::factory($row['f_type']);
			$myFeed->uid = $row['u_id'];
			...
			$myFeed->actor = new Core_User($row['u_id'], true);
			$myFeed->receiver = new Core_User($row['u_id_receive'], true);
	...
	*/
	public static function factory($type)
	{
		...
		switch($type)
		{
			case self::TYPE_STATUS: $myFeed = new Core_Backend_Feed_Status(); break;
			case self::TYPE_PROFILE_EDIT: $myFeed = new Core_Backend_Feed_ProfileEdit(); break;
			case self::TYPE_FOLLOW_ADD: $myFeed = new Core_Backend_Feed_FollowAdd(); break;
			case self::TYPE_GROUP_JOIN: $myFeed = new Core_Backend_Feed_GroupJoin(); break;
			default: $myFeed = new Core_Backend_Feed();
		}
	...
	*/
	...
	public function getDefaultData(Core_Backend_Feed $feed)
	{
		$this->id = $feed->id;
		$this->uid = $feed->uid;
	...
	public static function cacheGet($feedid, &$cacheSuccess = false, $forceStore = false)
	{
		...
		if(!$row || isset($_GET['live']) || $forceStore)
		{
			...
			$sql = 'SELECT 	f_id, f_type, f_serialized_data, 
							...
							f.u_id, f.u_id_receive
					FROM ' . TABLE_PREFIX . 'feed f
					WHERE f_id = ? ';
			$row = $db3->query($sql, array($feedid))->fetch();
			if($row['f_id'] > 0)
			{
				$myFeed = Core_Backend_Feed::factory($row['f_type']);
				$myFeed->uid = $row['u_id'];
		...
		else
		{
			...
			$myFeed = Core_Backend_Feed::factory($row[1]);
			...
			$myFeed->actor = new Core_User($row[9], true);
			$myFeed->receiver = new Core_User($row[10], true);
			$myFeed->updateActiveActor();
			$myFeed->mobileversionUrl();
